...
When a figure has direct sibling PNG alternatives in `teaching-frames/`, choose **Swap pose** or **Swap teaching pose** to compare them with `selected.png`. Choosing an alternative exchanges the two files atomically: future generation still reads `selected.png`, while the previous selection remains in the alternative slot and can be restored by repeating the swap. The action does not generate artwork or change the current card, approval, rework marker, or notes.
