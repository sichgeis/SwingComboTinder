...
- Progress: Added filesystem-versioned media URLs, no-store Studio responses, an ordered single-flight refresh coordinator, figure-scoped live synchronization, draft-safe Content reload/preview behavior, and transcript mutation events.
- Validation: `task check` passes with 22 Python maintenance tests, 25 application test files and 80 tests plus the production PWA build; all three Studio browser modules pass `node --check`. Focused tests cover atomic same-path asset replacement and overlapping refresh coalescing. A no-generation real 42-card Studio API run returned `cache-control: no-store` and distinct version tokens on Lindy Circle's mutable pose and current-master URLs.
