...
- Status: Done
- Approved: 2026-07-18
- Goal: Make completed Studio mutations and changed image pixels appear everywhere immediately without a manual browser refresh.
- Scope: Content-versioned media URLs; ordered/coalesced catalog refreshes; draft-safe active Content synchronization; transcript mutation events; cache controls; tests and documentation.
- Non-goals: Do not add filesystem watching, multi-user editing, optimistic mutation schemas, dependencies, automatic conflict resolution, or paid validation requests.
- Specification: [`specs/studio-synchronization-repair.md`](specs/studio-synchronization-repair.md)
- Progress: Added filesystem-versioned media URLs, no-store Studio responses, an ordered single-flight refresh coordinator, figure-scoped live synchronization, draft-safe Content reload/preview behavior, and transcript mutation events. Image approval now consumes the current session's **new** marker and immediately collapses the completed card; later candidates or rework still reopen it.
- Validation: `task check` passes with 22 Python maintenance tests, 26 application test files and 82 tests plus the production PWA build; Studio browser modules pass `node --check`. Focused tests cover atomic same-path asset replacement, overlapping refresh coalescing, approval-marker consumption, and the remaining expansion rules. A no-generation real 42-card Studio API run returned `cache-control: no-store` and distinct version tokens on Lindy Circle's mutable pose and current-master URLs.
- Next action: None; use the Studio normally and report any workflow that still needs a manual refresh.
